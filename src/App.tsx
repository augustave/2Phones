...
export default function App() {
  ...
  return (
    ...
    >
      {/* Navigation Controls */}
      <div className={`absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 sm:px-12 z-50 transition-opacity duration-1000 ${isLoaded ? "opacity-100" : "opacity-0"}`}>
        <button 
          onClick={prevSlide}
          className="p-3 sm:p-4 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur-md transition-all border border-white/5 disabled:opacity-30 disabled:pointer-events-none cursor-pointer"
        >
      ...
      <div className="relative w-full max-w-[1400px] h-[800px] flex items-center justify-center perspective-[2000px]">
        {portfolioData.map((item, index) => {
          ...
          return (
            ...
            >
              {/* Left Phone */}
              <div className={`scale-[0.65] sm:scale-75 md:scale-90 xl:scale-100 -mt-24 xl:-mt-12 transition-all duration-1000 delay-0`}>
      ...
      <div className={`absolute bottom-8 sm:bottom-12 flex gap-3 z-50 transition-opacity duration-1000 ${isLoaded ? "opacity-100" : "opacity-0"}`}>
